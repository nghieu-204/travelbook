      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial: { id: number; name: string; avatar: string; role: string; rating: number; content: string; tour_id: number; user_avatar: string; user_name: string; comment: string; tour_name: string }) => (
            <Link href={`/tours/${testimonial.tour_id}`} key={testimonial.id} className="bg-white p-8 rounded-3xl shadow-[0_2px_20px_rgba(0,0,0,0.04)] border border-slate-100 flex flex-col h-full hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] transition-all group">
