...
const DualRangeSlider = ({ value, onChange }: { value: [number, number], onChange: (val: [number, number]) => void }) => {
  ...
  const minPercent = Math.max(0, Math.min(100, ((value[0] - min) / (max - min)) * 100));
  const maxPercent = Math.max(0, Math.min(100, ((value[1] - min) / (max - min)) * 100));
...
export default function TourFilter({
  ...
  const debouncedPush = useDebouncedCallback(() => {
      ...
      if (filterParams.priceRange[1] < 20000000) params.set('maxPrice', filterParams.priceRange[1].toString())
  ...
  useEffect(() => {
    ...
    debouncedPush();
  }, [filterParams, debouncedPush])

  const filteredDestinations = allDestinations.filter(d => d.toLowerCase().includes(destSearch.toLowerCase()));

  if (isLoading) return (
    <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm sticky top-24 h-[800px] animate-pulse">
          ...
          <div className="flex bg-slate-100 rounded-lg p-1">
            {['Tất cả', 'Trong nước', 'Quốc tế'].map((cat) => (
              <button
                ...
                className={`flex-1 text-[13px] font-bold py-2 rounded-md transition-all ${filterParams.category === cat
