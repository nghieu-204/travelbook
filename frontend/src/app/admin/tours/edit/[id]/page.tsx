...
export default function EditTourV2() {
  ...
  const [metadata, setMetadata] = useState<Metadata>({
    categories: [], regions: [], destinations: [], tourTypes: [], occasions: []
  })
  ...
  const loadMetadataAndTour = async () => {
    try {
      const [metaData, tourData] = await Promise.all([
        fetchApi('/metadata'),
      ...
      if (tourData) {
        setTitle(tourData.name || tourData.title || '');
        setDescription(tourData.description || '');
        setStatus(tourData.status || 'Active');
        
        if (tourData.departure_date || tourData.start_date) {
            setStartDate(new Date(tourData.departure_date || tourData.start_date).toISOString().split('T')[0]);
        }
        ...
        const cPrice = tourData.child_price || 0;
        setPriceChild(cPrice.toString());
        setPriceChildStr(cPrice.toLocaleString('vi-VN'));
        
        setMaxSeats((tourData.available_spots || tourData.max_seats || 30).toString());
        ...
        if (tourData.image) {
          setMainImage({ preview: tourData.image });
        }
        
        if (tourData.gallery) {
            try {
                const g = typeof tourData.gallery === 'string' ? JSON.parse(tourData.gallery) : tourData.gallery;
                if (Array.isArray(g)) {
                    setGalleryImages(g.map(img => ({ preview: img.url || img })));
                }
            } catch(e) {}
        }
        ...
        if (tourData.itinerary) {
            try {
                const it = typeof tourData.itinerary === 'string' ? JSON.parse(tourData.itinerary) : tourData.itinerary;
                if (Array.isArray(it) && it.length > 0) {
                    setItinerary(it.map((item: any, idx: number) => ({
                        id: item.id || idx,
                        title: item.title || '',
        ...
        const cat = metaData.categories.find(c => c.name === tourData.category);
        if (cat) setCategoryId(cat.id.toString());
        
        const reg = metaData.regions.find(r => r.name === tourData.region);
        if (reg) setRegionId(reg.id.toString());
        
        const loc = metaData.destinations.find(d => d.name === tourData.location);
        if (loc) setDestinationId(loc.id.toString());
      }
    } catch (error) {
  ...
  const filteredRegions = metadata.regions.filter(r => r.category_id.toString() === categoryId)
  const filteredDestinations = metadata.destinations.filter(d => d.region_id.toString() === regionId)
  ...
  const handleQuickAddDestination = async (name: string) => {
    ...
    try {
      ...
      if (res && res.destination) {
        setMetadata(prev => ({
          ...prev,
          destinations: [...prev.destinations, res.destination]
        }));
  ...
  const handleSave = async () => {
    ...
    try {
      await fetchApi(`/tours/${tourId}`, {
        ...
        data: {
          name: title,
          location: metadata.destinations.find(d => d.id.toString() === destinationId)?.name || '',
          region: metadata.regions.find(r => r.id.toString() === regionId)?.name || '',
          price: Number(priceAdult),
          child_price: Number(priceChild),
          original_price: Math.round(Number(priceAdult) * 1.2),
          available_spots: Number(maxSeats),
          departure_date: startDate,
          duration: `${days} Ngày ${nights} Đêm`,
          category: metadata.categories.find(c => c.id.toString() === categoryId)?.name || '',
          ...
          <section className="bg-[#1e293b] rounded-2xl border border-slate-800">
            ...
            <div className="p-6 grid grid-cols-3 gap-6">
              <div>
                ...
                <select value={categoryId} onChange={handleCategoryChange} className="w-full bg-[#0f172a] border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 outline-none">
              ...
              <div>
                ...
                <select value={regionId} onChange={handleRegionChange} disabled={!categoryId} className="w-full bg-[#0f172a] border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-blue-500 outline-none disabled:opacity-50">
              ...
              <div>
                ...
                <SearchableAdminDropdown 
                  options={filteredDestinations.map(d => ({ id: String(d.id), name: d.name }))}
                  value={destinationId}
